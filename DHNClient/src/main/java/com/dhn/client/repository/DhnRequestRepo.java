...
public interface DhnRequestRepo extends JpaRepository<DhnRequest, String>, JpaSpecificationExecutor<DhnRequest> {
	...
	final static String DEL_MSGID = "delete from TBL_REQUEST_DHN where MSGID = :msgid";

	final static String DELIN_MSGID = "delete from TBL_REQUEST_DHN where MSGID in :msgid";

	final static String REQ_SEND = "select * from TBL_REQUEST_DHN where RESERVE_DT < date_format(NOW(), '%Y%m%d%H%i%s') limit 0, 1000";

	final static String REQ_SEND_ORACLE = "select * from TBL_REQUEST_DHN where RESERVE_DT < to_char(sysdate, 'YYYYMMDDHH24MISS') and rownum <=1000";
